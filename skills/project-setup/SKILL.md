...
outputs:
  ...
  - name: setup_report
    type: html
    description: 初始化报告，按主 SKILL.md「报告输出规范」的 HTML 骨架输出
rules:
...
> **按分类模板 `assets/templates/reports/setup-report.html` 输出的 HTML 文件**（`.html`）。保留以下分区：项目信息（名称/类型/技术栈）、目录结构（代码树）、工具链配置（`<table>` 工具/配置文件/说明）、Git 配置（仓库/分支策略/保护规则）、待办事项清单。落盘建议：`reports/setup-<项目名>.html`，并在对话中预览交付。
